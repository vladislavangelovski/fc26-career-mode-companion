...
import type { AnalystState, Tactic } from '../src/shared/types'
...
function lowerAppPriority(lower: boolean) {
  if (process.platform !== 'win32') return
  for (const { pid } of app.getAppMetrics()) {
    try { setPriority(pid, lower ? constants.priority.PRIORITY_BELOW_NORMAL : constants.priority.PRIORITY_NORMAL) }
    catch { /* The process may have exited between collection and reprioritisation. */ }
  }
}
...
function createWindow() {
  ...
  window.on('blur', () => lowerAppPriority(true))
  window.on('focus', () => { lowerAppPriority(false); emit() })
  window.on('closed', () => { window = null })
}

function emit() { if (window?.isFocused()) window.webContents.send('career:changed', store.state) }
